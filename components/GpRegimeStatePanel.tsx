...
import type { Domain, OntologyMode, GpVariableName } from '@/lib/types'

const POLL = 30_000
const API_BASE =
  process.env.NEXT_PUBLIC_API_BASE_URL ?? 'http://localhost:8000'

const VAR_LABELS: Record<GpVariableName, string> = {
  ConflictIntensityElevated: 'CONFLICT_INTENS',
  TradeDisruptionRisk:       'TRADE_DISRUPT',
  SanctionsPressureElevated: 'SANCTIONS_PRESS',
  DiplomaticTensionHigh:     'DIPLOMATIC_TENS',
  SupplyChainStress:         'SUPPLY_CHN_STRSS',
  CurrencyWarSignal:         'CURRENCY_WAR',
  EnergyWeaponizationRisk:   'ENERGY_WEAPON',
  GlobalTradeVolumeWeak:     'TRADE_VOL_WEAK',
}

const VAR_ORDER: GpVariableName[] = [
...
type GpRegimeProbabilityMap = Record<GpVariableName, number>
...
async function fetchGpVariableProbabilities(
  [, domain, mode]: [string, Domain, OntologyMode],
): Promise<GpRegimeProbabilityMap> {
  ...
  return Object.fromEntries(entries) as GpRegimeProbabilityMap
}
...
export default function GpRegimeStatePanel({ domain, ontologyMode }: Props) {
  const { data, error } = useSWR<GpRegimeProbabilityMap>(
    ['gp-regime-probabilities', domain, ontologyMode],
    fetchGpVariableProbabilities,
    ...
    <div className="panel regime-panel">
      <div className="panel-label has-tip">
        ...
        <div className="tip" style={{ left: 0, transform: 'none' }}>
          Geopolitical regime variables for the current evidence cycle. Each variable
