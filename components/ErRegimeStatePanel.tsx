...
import type { Domain, OntologyMode, ErVariableName } from '@/lib/types'

const POLL = 30_000
const API_BASE =
  process.env.NEXT_PUBLIC_API_BASE_URL ?? 'http://localhost:8000'

const VAR_LABELS: Record<ErVariableName, string> = {
  OilPriceSurge:              'OIL_PRICE_SURGE',
  NatGasPriceSurge:           'NATGAS_SURGE',
  EnergyEquityMomentum:       'ENERGY_EQ_MOM',
  OPECSupplyConstraint:       'OPEC_SUPPLY_CNST',
  RenewablesDisplacement:     'RENEW_DISPLACE',
  EnergyInflationPersistent:  'ENERGY_INF_PERS',
  GeopoliticalRiskElevated:   'GEO_RISK_ELEV',
  DemandDestructionRisk:      'DEMAND_DESTR',
}

const VAR_ORDER: ErVariableName[] = [
  'OilPriceSurge',
  'NatGasPriceSurge',
...
type ErRegimeProbabilityMap = Record<ErVariableName, number>
...
async function fetchErVariableProbabilities(
  [, domain, mode]: [string, Domain, OntologyMode],
): Promise<ErRegimeProbabilityMap> {
  ...
  return Object.fromEntries(entries) as ErRegimeProbabilityMap
}
...
export default function ErRegimeStatePanel({ domain, ontologyMode }: Props) {
  const { data, error } = useSWR<ErRegimeProbabilityMap>(
    ['er-regime-probabilities', domain, ontologyMode],
    fetchErVariableProbabilities,
    ...
    <div className="panel regime-panel">
      <div className="panel-label has-tip">
        ...
        <div className="tip" style={{ left: 0, transform: 'none' }}>
          Energy regime variables for the current evidence cycle. Each variable
